...
/**
 * 推箱规划算法 v3_BFS - 纯 BFS+A* 版本（动态贪心选择）
 * 
 * 算法核心：
 ...
 *    - 对每个箱子-目标组合，找到最佳推箱位（4个方向中车最容易到达的）
 *    - 使用BFS计算车到推箱位的真实距离
 *    - 评分 = 车到最佳推箱位距离 + 箱子到目标距离
 *    - 选择评分最小的组合
 * 2. 对选中的箱子：
 ...
 *       - 评分公式：dist_target×10 + 拥挤度×5 + 反向惩罚 + dist_car×2
 *    c. 车移动到推箱位：使用全局BFS+A*
 *    d. 遇到死点需换推箱位：使用全局BFS+A*绕箱子
 * 3. 推完一个箱子后，回到步骤1，重新选择下一个最优组合
 * 4. 所有路径搜索统一使用全局BFS+A*，无预分配，真正的动态贪心
 *
