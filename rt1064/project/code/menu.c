...
static const PlannerBoxPathOutput *menu_req_box_info(size_t box_idx, const PlannerBoxPathOutput *fallback)
{
    if (box_idx < special_paths.box_count && box_idx < 10)
    {
        return &special_paths.box_paths[box_idx];
    }
    return fallback;
}

static const char *menu_req_usage_text(const PlannerBoxPathOutput *box_path)
{
    int mask = 0;
    if (!box_path->dropped_req1) mask |= 1;
    if (!box_path->dropped_req2) mask |= 2;
    if (!box_path->dropped_req3) mask |= 4;
    switch (mask)
    {
    case 1: return "1";
    case 2: return "2";
...
static menu_item_t path_special;
...
static const menu_item_t *const path_children[] = {&path_first, &path_final, &path_special, &path_car, &path_easy};
...
void menu_enter_selected(void)
{
    ...
    switch (item->type)
    {
        ...
        case MENU_ITEM_VIEW:
            ...
            menu_rt.need_clear = 1;
            // 进入路径视图时重置页面
            if (item == &path_first || item == &path_final || item == &path_special || item == &path_car)
            {
                path_text_page = 0;
                path_graph_page = 0;
            }
...
void menu_handle_view_keys(key_state_enum k1, key_state_enum k2, key_state_enum k3, key_state_enum k4)
{
    ...
    // 处理path_first/path_final/path_special视图（箱子路径）
    else if (menu_rt.active_item == &path_first || menu_rt.active_item == &path_final || menu_rt.active_item == &path_special)
    {
        const PlannerAllBoxPaths *paths =
            (menu_rt.active_item == &path_first) ? &first_paths :
            (menu_rt.active_item == &path_final) ? &final_paths :
            &special_paths;
        ...
        if (path_display_mode == 0)
        {
            ...
            else if (path_text_page >= total_pages)
            {
                path_text_page = total_pages - 1;
            }
            if (k1 == KEY_SHORT_PRESS || k1 == KEY_LONG_PRESS)
            {
                if (path_text_page > 0)
        ...
        else
        {
            ...
            else if (path_graph_page >= total_pages)
            {
                path_graph_page = total_pages - 1;
            }
            if (k1 == KEY_SHORT_PRESS || k1 == KEY_LONG_PRESS)
            {
...
void draw_main_info(void)
{
    ips200_show_string(100, 16, "plan");
    ips200_show_int(140, 16, res, 3);
    ips200_show_int(180, 16, last_err_stage, 3);
    if(last_err_detail == 131){
        ips200_show_int(200, 16, last_special_path_fail_reason, 3);
    }
    else{
    ...
    uint16 by = 64;
    for (size_t i = 0; i < special_paths.box_count && i < 10; i++) {
        if (special_paths.box_paths[i].has_bomb_exploded) {
            ips200_show_string(140, by, "Bomb:");
            ips200_show_uint(190, by,
                (uint16)special_paths.box_paths[i].bomb_exploded_at.row, 2);
            ips200_show_uint(206, by,
                (uint16)special_paths.box_paths[i].bomb_exploded_at.col, 2);
            by += 16;
        }
        
        for (int j = 0; j < special_paths.box_paths[i].bomb_extra_exploded_count && j < 5; j++) {
            if(j == 0){
                ips200_show_string(140, by, "BombB:");
            }
            ips200_show_uint(190, by,
                (uint16)special_paths.box_paths[i].bomb_extra_exploded_at[j].row, 2);
            ips200_show_uint(206, by,
                (uint16)special_paths.box_paths[i].bomb_extra_exploded_at[j].col, 2);
            by += 16;
        }
        /* 每个箱子都显示要求使用情况，不依赖是否进行了特殊路径 */
        ips200_show_string(140, by, "B");
        ips200_show_uint(148, by, (uint16)i, 1);
        ips200_show_string(190, by, menu_req_usage_text(&special_paths.box_paths[i]));
        by += 16;
    }
...
void draw_path_first(void)
{
    if (path_display_mode == 0)
    {
        ...
        ips200_show_string(0, 272, "Req:");
        ips200_show_string(48, 272, menu_req_usage_text(menu_req_box_info(current_box_idx, box_path)));
...
// 特殊路径显示：结构与 draw_path_first 一致，但数据源为 special_paths
void draw_path_special(void)
{
    if (path_display_mode == 0)
    {
        ...
        for (size_t i = 0; i < special_paths.box_count && i < 5; i++)
        {
            if (special_paths.box_paths[i].valid && special_paths.box_paths[i].path_len > 0)
            {
                uint16 box_pages = (special_paths.box_paths[i].path_len + items_per_page - 1) / items_per_page;
                if (current_page < accumulated_pages + box_pages)
                {
                    current_box_idx = i;
                    box_page_offset = current_page - accumulated_pages;
        ...
        PlannerBoxPathOutput *box_path = &special_paths.box_paths[current_box_idx];
        if (!box_path->valid || box_path->path_len == 0)
        {
            return;
        }
        ...
        // 显示页面信息和箱子编号，以及该箱子实际生效的要求
        uint16 total_pages = calculate_text_total_pages(&special_paths);
        ips200_show_string(0, 0, "Box");
        ips200_show_uint(32, 0, current_box_idx, 1);
        ips200_show_string(48, 0, "Page");
        ips200_show_uint(88, 0, path_text_page + 1, 2);
        ...
        for (size_t i = start_in_box; i < end_in_box; i++)
        {
            ...
            int is_obstacle = box_path->has_obstacle_to_clear &&
                box_path->path[i].row == box_path->obstacle_to_clear.row &&
                box_path->path[i].col == box_path->obstacle_to_clear.col;
            int is_bomb_on_path = 0;
            for (int b = 0; b < box_path->bomb_on_path_count && b < 5; b++) {
                if (box_path->path[i].row == box_path->bomb_on_path[b].row &&
                    box_path->path[i].col == box_path->bomb_on_path[b].col) {
                    is_bomb_on_path = 1;
                    break;
                }
            }
            ...
            else if (is_bomb_on_path)
                ips200_set_color(RGB565_BLUE, RGB565_BLACK);
            ips200_show_uint(x, y, i, 3);
            ips200_show_uint(x + 32, y, box_path->path[i].row, 3);
            ips200_show_uint(x + 64, y, box_path->path[i].col, 3);
            if (is_obstacle || is_bomb_on_path)
                ips200_set_color(RGB565_WHITE, RGB565_BLACK);
        }
    }
    else
    {
        ...
        for (size_t i = 0; i < special_paths.box_count && i < 5; i++)
        {
            if (special_paths.box_paths[i].valid && special_paths.box_paths[i].path_len >= 2)
            {
                if (valid_box_count == path_graph_page)
                {
                    current_box_idx = i;
        ...
        if (box_path->valid && box_path->path_len >= 2)
        {
            ...
            // 绘制该箱子的路径（连接所有点），目标障碍段用红色，经过的炸弹段用蓝色
            for (size_t i = 0; i < box_path->path_len - 1; i++)
            {
                uint16 x1 = box_path->path[i].col * scale;
                uint16 y1 = box_path->path[i].row * scale + offset_y;
                uint16 x2 = box_path->path[i + 1].col * scale;
                uint16 y2 = box_path->path[i + 1].row * scale + offset_y;
                uint16 seg_color = color;
                if (box_path->has_obstacle_to_clear)
                {
                    int p1_obs = (box_path->path[i].row == box_path->obstacle_to_clear.row &&
                                  box_path->path[i].col == box_path->obstacle_to_clear.col);
                    int p2_obs = (box_path->path[i + 1].row == box_path->obstacle_to_clear.row &&
                                  box_path->path[i + 1].col == box_path->obstacle_to_clear.col);
                    if (p1_obs || p2_obs)
                        seg_color = RGB565_RED;
                }
                if (seg_color == color && box_path->bomb_on_path_count > 0)
                {
                    int p1_bomb = 0, p2_bomb = 0;
                    for (int b = 0; b < box_path->bomb_on_path_count && b < 5; b++)
                    {
                        if (box_path->path[i].row == box_path->bomb_on_path[b].row &&
                            box_path->path[i].col == box_path->bomb_on_path[b].col) p1_bomb = 1;
                        if (box_path->path[i + 1].row == box_path->bomb_on_path[b].row &&
                            box_path->path[i + 1].col == box_path->bomb_on_path[b].col) p2_bomb = 1;
                    }
                    if (p1_bomb || p2_bomb)
                        seg_color = RGB565_BLUE;
                }
                if (x1 < 240 && y1 < 320 && x2 < 240 && y2 < 320)
                    ips200_draw_line(x1, y1, x2, y2, seg_color);
            }
            
            // 在目标障碍位置绘制红色标记点
            if (box_path->has_obstacle_to_clear)
            {
                uint16 ox = box_path->obstacle_to_clear.col * scale;
                uint16 oy = box_path->obstacle_to_clear.row * scale + offset_y;
                if (ox < 240 && oy < 320)
                {
                    for (int d = -1; d <= 1; d++)
                    {
                        if (ox + d >= 0 && ox + d < 240)
                            ips200_draw_point(ox + d, oy, RGB565_RED);
                        if (oy + d >= 0 && oy + d < 320)
                            ips200_draw_point(ox, oy + d, RGB565_RED);
                    }
                }
            }
            // 在特殊路径经过的炸弹位置绘制蓝色标记点
            for (int b = 0; b < box_path->bomb_on_path_count && b < 5; b++)
            {
                uint16 bx = box_path->bomb_on_path[b].col * scale;
                uint16 by = box_path->bomb_on_path[b].row * scale + offset_y;
                if (bx < 240 && by < 320)
                {
                    for (int d = -1; d <= 1; d++)
                    {
                        if (bx + d >= 0 && bx + d < 240)
                            ips200_draw_point(bx + d, by, RGB565_BLUE);
                        if (by + d >= 0 && by + d < 320)
                            ips200_draw_point(bx, by + d, RGB565_BLUE);
                    }
                }
            }
            
            // 在倒数第4行显示箱子编号、炸弹目标及该箱子实际生效的要求
            ips200_show_string(0, 256, "Box");
            ips200_show_uint(32, 256, current_box_idx, 1);
            uint16 total_pages = calculate_graph_total_pages(&special_paths);
            ips200_show_string(64, 256, "Page");
            ips200_show_uint(104, 256, path_graph_page + 1, 1);
            ips200_show_string(112, 256, "/");
            ips200_show_uint(120, 256, total_pages, 1);
