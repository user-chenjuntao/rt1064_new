...
| 代价项 | 权重 | 计算方式 | 目的 | 优先级 |
|--------|------|----------|------|--------|
| `dist_after` | ×10 | **真实距离** 或 曼哈顿距离(推后位置, 目标) | 确保箱子朝目标移动 | 🔴 最高 |
| `adj_pen` | ×5 | 统计推后位置周围4方向的障碍物/箱子数 | 避免推到狭窄位置 | 🟡 中等 |
| `reverse_pen` | 5^n | n=连续反向次数，5的n次方 | 防止来回推动震荡 | 🟠 递增 |
| `car_to_push` | ×2 | 曼哈顿距离(车, 推箱位) | 优化车的移动效率 | 🟢 较低 |

**注意**：`dist_after` 的计算方式取决于是否启用全局路径规划：
- **启用时**（远距离场景）：使用全局BFS预计算的真实距离 `dist[new_box_row][new_box_col]`
  * 真实距离考虑了实际障碍物和箱子的阻挡
  * 如果某方向推后无法到达目标（dist=∞），评分会极高，自动避免死路
  * 评分更准确，避免曼哈顿距离的误导
- **未启用时**（近距离场景）：使用快速的曼哈顿距离计算
  * 计算简单高效，适合近距离场景
  * 假设可以直线到达，不考虑障碍物
